...
layui.use(['jquery','form','util'], function () {
    ...
    if (user!=null && user!='') {
    	...
    	$(".layui-form-radio").get(user.sex==null || user.sex=='' || user.sex=='undefined' ? "男" : user.sex ).click();
    } else {
	...
	$("#upload-file").change(function(){
		...
		$.ajax({
    		...
    		data: data,
    		async:true,
    		url: "/user/upload.do?token="+$("#token").val()+"&uid="+$("#uid").val(),
    		cache: false,  
            contentType: false,  
            processData: false,
    		success:function(result) {
    			if (result.isOk) {
    				...
    				$("#upload-img").attr("src","/images"+user.img+"?"+Math.random());
    				$(".blog-user img").attr("src","/images"+user.img+"?"+Math.random());
    			} else {
    				layer.msg(result.msg,{anim:6});
    				layer.close(index);
    			}
